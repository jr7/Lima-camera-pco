...
void Camera::_init(){
...

	{
		...
		PCO_FN7(error, pcoFn, PCO_SetDateTime, m_handle, ucDay, ucMonth, wYear, wHour, ucMin, ucSec);
...
const char *Camera::_pco_SetTransferParameter_SetActiveLookupTable(int &error){
	...
	if (m_pcoData->stcPcoCamType.wInterfaceType==INTERFACE_CAMERALINK) 
	{
			...
			(_pcoData.clTransferParam.Transmit != m_pcoData->clTransferParam.Transmit)	)
		{
			...
			if(error){
				sprintf_s(msg,ERRMSG_SIZE, "PCO_SetTransferParameter - baudrate[%lu][%lu] dataFormat[x%08lx][x%08lx] trasmit[%lu][%lu]",
...
const char *Camera::_pco_GetCameraType(int &error){
	...
	// --- Get camera type
	{
		...
		sprintf_s(m_pcoData->camera_name, CAMERA_NAME_SIZE, "%s %s (SN %lu)", 
...
void Camera::_pco_set_shutter_rolling_edge(int &error){
	...
	msg = msgBuff;
	sprintf_s((char *) msg, MSG_SIZE, "[Sleep %lu ms]", sleepMs);
	DEB_ALWAYS() << fnId << " " << msg;
	::Sleep(sleepMs);
